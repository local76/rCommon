...
pub fn start_service(service_name: &str) -> Result<(), String> {
    if !has_admin_privileges() {
        return Err("Privilege check failed: Administrator / root privileges are required to modify system services.".to_string());
    }
    ...
    unsafe {
        ...
        if scm.is_null() {
            return Err(format!("Failed to open SC Manager: OS error {}", std::io::Error::last_os_error()));
        }
        ...
        if svc.is_null() {
            ...
            windows_sys::Win32::System::Services::CloseServiceHandle(scm);
            return Err(format!("Failed to open service: {}", err));
        }
        ...
        if ok != 0 {
            ...
            Err(format!("Failed to start service: {}", std::io::Error::last_os_error()))
        }
    }
        ...
        match detect_init_system() {
            InitSystem::Systemd => {
                ...
                match status {
                    ...
                    Ok(s) => Err(format!("systemctl start failed with status: {}", s)),
                    Err(e) => Err(format!("Failed to execute systemctl: {}", e)),
                }
            }
            InitSystem::OpenRc => {
                ...
                match status {
                    Ok(s) if s.success() => Ok(()),
                    Ok(s) => Err(format!("rc-service start failed with status: {}", s)),
                    Err(e) => Err(format!("Failed to execute rc-service: {}", e)),
                }
            }
            InitSystem::None => Err("No supported init system detected (systemd or openrc required)".to_string()),
        }
    }
...
pub fn stop_service(service_name: &str) -> Result<(), String> {
    ...
    unsafe {
        ...
        if ok != 0 {
            ...
            Err(format!("Failed to stop service: {}", std::io::Error::last_os_error()))
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        match detect_init_system() {
            InitSystem::Systemd => {
                ...
                match status {
                    Ok(s) if s.success() => Ok(()),
                    Ok(s) => Err(format!("systemctl stop failed with status: {}", s)),
                    Err(e) => Err(format!("Failed to execute systemctl: {}", e)),
                }
            }
            InitSystem::OpenRc => {
                ...
                match status {
                    Ok(s) if s.success() => Ok(()),
                    Ok(s) => Err(format!("rc-service stop failed with status: {}", s)),
                    Err(e) => Err(format!("Failed to execute rc-service: {}", e)),
                }
            }
            InitSystem::None => Err("No supported init system detected (systemd or openrc required)".to_string()),
    ...
    #[cfg(target_os = "linux")]
    {
        match detect_init_system() {
            InitSystem::Systemd => {
                ...
                match status {
                    Ok(s) if s.success() => Ok(()),
                    Ok(s) => Err(format!("systemctl restart failed with status: {}", s)),
                    Err(e) => Err(format!("Failed to execute systemctl: {}", e)),
                }
            }
            InitSystem::OpenRc => {
                ...
                match status {
                    Ok(s) if s.success() => Ok(()),
                    Ok(s) => Err(format!("rc-service restart failed with status: {}", s)),
                    Err(e) => Err(format!("Failed to execute rc-service: {}", e)),
                }
            }
            InitSystem::None => Err("No supported init system detected (systemd or openrc required)".to_string()),
